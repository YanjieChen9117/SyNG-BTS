...
# -*- coding: utf-8 -*-
"""
完整的SyNG-BTS数据增强workflow脚本
处理4种癌症类型 × 3种标准化方法 × 20个batch × 2种增强方法 = 480次实验
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/Users/yanjiechen/Documents/Github/SyNG-BTS/syng_bts/python/workflow.log'),
...
def write_experiment_record(record_file, cancer_type, normalization_method, batch_num, 
    ...
    # 创建记录行
    record_line = f"{cancer_type}-{normalization_method}-batch{batch_num}-{off_aug_method}-{status}|{timestamp}|{duration_str}|{completion_ratio:.1f}%|{completed_experiments}/{total_experiments}\n"
...
def run_syng_bts_experiment(output_path, cancer_type, normalization_method, batch_num, 
    ...
    logger.info(f"开始实验: {cancer_type}_{normalization_method}_batch_{batch_num}_{off_aug_method}")
    ...
    try:
        ...
        logger.info(f"实验完成: {cancer_type}_{normalization_method}_batch_{batch_num}_{off_aug_method}")
        ...
        logger.error(f"实验失败: {cancer_type}_{normalization_method}_batch_{batch_num}_{off_aug_method}, 错误: {str(e)}")
...
def process_single_dataset(data_path, cancer_type, normalization_method, base_output_path, 
    ...
    for batch_num in range(1, 21):
        ...
        # 运行两种增强方法的实验
        for off_aug_method in ["AE_head", None]:
...
def main():
    ...
    logger.info("开始完整的SyNG-BTS workflow")
    ...
    # 创建实验记录文件
    record_file = "/Users/yanjiechen/Documents/Github/SyNG-BTS/syng_bts/python/experiment_records.txt"
    
    # 初始化记录文件
    with open(record_file, 'w', encoding='utf-8') as f:
        f.write("SyNG-BTS实验记录\n")
    ...
    data_files = {
        "KICH": {
            "raw": "KICHSubtype_lymph_nodes_examined.csv",
            "TC": "KICHSubtype_lymph_nodes_examined_TC.csv", 
            "DESeq": "KICHSubtype_lymph_nodes_examined_DESeq.csv"
        },
        "PAAD": {
            "raw": "PAADPositive_4.csv",
            "TC": "PAADPositive_4_TC.csv",
            "DESeq": "PAADPositive_4_DESeq.csv"
        },
        "READ": {
            "raw": "READPositive_4.csv",
            "TC": "READPositive_4_TC.csv", 
            "DESeq": "READPositive_4_DESeq.csv"
        },
        "UCS": {
            "raw": "UCSSubtype_primary_pathology_total_pelv_lnr.csv",
            "TC": "UCSSubtype_primary_pathology_total_pelv_lnr_TC.csv",
            "DESeq": "UCSSubtype_primary_pathology_total_pelv_lnr_DESeq.csv"
        }
    }
    
    total_experiments = len(cancer_types) * len(normalization_methods) * 20 * 2
    ...
    logger.info("完整的SyNG-BTS workflow完成！")
